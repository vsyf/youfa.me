...
const routes: Post[] = router.getRoutes()
  ...
  .map(i => ({
    ...
    no_list: i.meta.frontmatter.no_list,
    no_render: i.meta.frontmatter.no_render,
  }))

const posts = computed(() =>
  [...(props.posts || routes), ...props.extra || []]
    .sort((a, b) => +new Date(b.date) - +new Date(a.date))
    .filter(i => !i.no_list && !i.no_render)
...
<template>
  <ul>
    ...
    <template v-for="route, idx in posts" :key="route.path">
      ...
      >
        <span absolute left--3rem top--2rem text-8em font-bold color-transparent text-stroke-2 text-stroke-hex-aaa op10>{{
        ...
        >
          <li class="no-underline" flex="~ col md:row gap-2 md:items-center">
            ...
            <div flex="~ gap-2 items-center">
              <span
                v-if="route.redirect" i-carbon-arrow-up-right ml--1 mt--1 flex-none align-middle text-xs op50
                title="External"
              />
              <span v-if="route.inperson" i-ri:group-2-line flex-none align-middle op50 title="In person" />
              <span
                v-if="route.recording || route.video" i-ri:film-line flex-none align-middle op50
                title="Provided in video"
              />
              <span v-if="route.radio" i-ri:radio-line flex-none align-middle op50 title="Provided in radio" />
